...
<script type="text/javascript">  
	...
	function chekToPass(index){
    	...
    	if (rows.length==1){    
            var url = '<%=basePath %>chekToPassMoney.action?checkId='+rows[0].id+'&checkName='+rows[0].applicationName+'&checkNumber='+rows[0].applicationNumber+'&checkCategory='+rows[0].applicationCategory+'&checkWhat='+rows[0].applicationWhat+'&checkTime='+rows[0].applicationTime;
	...
	$('#commUserDatagrid').datagrid({  
            ...
            collapsible : true,//显示可折叠按钮 
            toolbar:"#tb",//在添加 增添、删除、修改操作的按钮要用到这个  
            url:'getAllMoneyApplication.action?keyword='+value,//url调用Action方法  
            loadMsg : '数据装载中......',  
            singleSelect: false, //允许选择多行  
            selectOnCheck: true,//true勾选会选择行，false勾选不选择行, 1.3以后有此选项。重点在这里  
            checkOnSelect: true, //true选择行勾选，false选择行不勾选, 1.3以后有此选项  
        ...
        if (selRow.length==0) {  
            ...
            $.messager.confirm('提示', '是否删除选中数据?', function (r) {  
                ...
                $.ajax({  
                    type: "POST",  
                    async: false,  
                    url: "deleteMoneyApplicationByIds.action?applicationIds=" + temID,  
                    data: temID,  
	...
	<table id="commUserDatagrid">  
       <thead>  
            <tr>  
                ...
                <th data-options="field:'applicationName',width:100,align:'center'">申请人姓名</th>  
