...
<script type="text/javascript">  
    $(function() {  
        $('#commUserDatagrid').datagrid({  
            ...
            url:'getAllGoodsApplication.action',//url调用Action方法  
            loadMsg : '数据装载中......',  
	...
	function delete2(index){
    	...
    	if (rows.length==1){    
            var url = '<%=basePath %>deleteGoodsApplicationById.action?applicationId='+rows[0].id;
	...
	$('#commUserDatagrid').datagrid({  
            ...
            collapsible : true,//显示可折叠按钮 
            toolbar:"#tb",//在添加 增添、删除、修改操作的按钮要用到这个  
            url:'getAllGoodsApplication.action?keyword='+value,//url调用Action方法  
            loadMsg : '数据装载中......',  
            singleSelect:true,//为true时只能选择单行 
            nowrap:false,  
            fitColumns:true,//允许表格自动缩放，以适应父容器   
  ...
  <body>
  <s:form  action="applicationGoodsSearch.action" method="post" enctype="multipart/form-data" theme="simple" >
